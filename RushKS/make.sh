...
#PID from: http://gis.publicaccessnow.com/arcgis/rest/services/RushKs/RushKsDynamic/MapServer/0
# Download using github.com/openaddresses/esri-dump

#PID => Address
#http://www.rush.kansasgis.com/ParcelDetail.aspx?parcel={PARCEL}&quickref=R2
...
curl -s 'http://www.rush.kansasgis.com' -c /tmp/make_rush_cookies.txt &>/dev/null
curl -s -X POST -d "" 'http://www.rush.kansasgis.com/Disclaimer.aspx' -b /tmp/make_rush_cookies.txt -c /tmp/make_rush_pass_cookies.txt &>/dev/null

while read -r LINE; do
    CENTRE=$(echo $LINE | jq -r -c '.geometry | .coordinates' | sed -e 's/\[//' -e 's/]//')
    PID=$(echo "083$(echo $LINE | jq -r -c '.properties | .PID')")
    ...
    if [[ ! -z $DEBUG ]]; then
        echo "-----"
        echo "WEB: http://www.rush.kansasgis.com/ParcelDetail.aspx?parcel=${PID}&quickref=R2"
    fi

    WEB=$(curl -s "http://www.rush.kansasgis.com/ParcelDetail.aspx?parcel=${PID}&quickref=R2" -b /tmp/make_rush_pass_cookies.txt)
    ...
    if [[ ! -z $(echo $WEB | grep "GeneralInfo_content_gvwPropertySitusInfo_lblSitusAddress_0" | grep -Eo "></span>") ]]; then
        echo "address field is blank"
        continue
    fi
