...
#PID => Address
#http://www.lane.kansasgis.com/ParcelDetail.aspx?parcel={PARCEL}&quickref=R2
...
curl -s 'http://www.lane.kansasgis.com' -c /tmp/make_lane_cookies.txt &>/dev/null
curl -s -X POST -d "" 'http://www.lane.kansasgis.com/Disclaimer.aspx' -b /tmp/make_lane_cookies.txt -c /tmp/make_lane_pass_cookies.txt &>/dev/null
...
while read -r LINE; do
    ...
    if [[ ! -z $DEBUG ]]; then
        echo "-----"
        echo "WEB: http://www.lane.kansasgis.com/ParcelDetail.aspx?parcel=${PID}&quickref=R2"
    fi

    WEB=$(curl -s "http://www.lane.kansasgis.com/ParcelDetail.aspx?parcel=${PID}&quickref=R2" -b /tmp/make_lane_pass_cookies.txt)
