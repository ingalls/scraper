...
#PID from: http://gis.publicaccessnow.com/arcgis/rest/services/OsageKs/OsageKsDynamic/MapServer/0
# Download using github.com/openaddresses/esri-dump

#PID => Address
#http://www.osage.kansasgis.com/ParcelDetail.aspx?parcel={PARCEL}&quickref=R2
...
curl -s 'http://www.osage.kansasgis.com' -c /tmp/make_osage_cookies.txt &>/dev/null
curl -s -X POST -d "" 'http://www.osage.kansasgis.com/Disclaimer.aspx' -b /tmp/make_osage_cookies.txt -c /tmp/make_osage_pass_cookies.txt &>/dev/null

while read -r LINE; do
    CENTRE=$(echo $LINE | jq -r -c '.geometry | .coordinates' | sed -e 's/\[//' -e 's/]//')
    PID=$(echo "070$(echo $LINE | jq -r -c '.properties | .PIDNO')")
    ...
    if [[ ! -z $DEBUG ]]; then
        echo "-----"
        echo "WEB: http://www.osage.kansasgis.com/ParcelDetail.aspx?parcel=${PID}&quickref=R2"
    fi

    WEB=$(curl -s "http://www.osage.kansasgis.com/ParcelDetail.aspx?parcel=${PID}&quickref=R2" -b /tmp/make_osage_pass_cookies.txt)
