...
#PID => Address
#http://www.franklin.kansasgis.com/ParcelDetail.aspx?parcel={PARCEL}&quickref=R2
...
curl -s 'http://www.franklin.kansasgis.com' -c /tmp/make_franklin_cookies.txt &>/dev/null
curl -s -X POST -d "" 'http://www.franklin.kansasgis.com/Disclaimer.aspx' -b /tmp/make_franklin_cookies.txt -c /tmp/make_franklin_pass_cookies.txt &>/dev/null
...
while read -r LINE; do
    ...
    if [[ ! -z $DEBUG ]]; then
        echo "-----"
        echo "WEB: http://www.franklin.kansasgis.com/ParcelDetail.aspx?parcel=${PID}&quickref=R2"
    fi

    WEB=$(curl -s "http://www.franklin.kansasgis.com/ParcelDetail.aspx?parcel=${PID}&quickref=R2" -b /tmp/make_franklin_pass_cookies.txt)
