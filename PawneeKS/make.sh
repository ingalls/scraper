...
#PID from: http://gis.publicaccessnow.com/arcgis/rest/services/LaneKs/LaneKsDynamic/MapServer/0
# Download using github.com/openaddresses/esri-dump

#PID => Address
#http://www.pawnee.kansasgis.com/ParcelDetail.aspx?parcel={PARCEL}&quickref=R2
...
curl -s 'http://www.pawnee.kansasgis.com' -c /tmp/make_pawnee_cookies.txt &>/dev/null
curl -s -X POST -d "" 'http://www.pawnee.kansasgis.com/Disclaimer.aspx' -b /tmp/make_pawnee_cookies.txt -c /tmp/make_pawnee_pass_cookies.txt &>/dev/null

while read -r LINE; do
    CENTRE=$(echo $LINE | jq -r -c '.geometry | .coordinates' | sed -e 's/\[//' -e 's/]//')
    PID=$(echo $LINE | jq -r -c '.properties | .PIN')
    ...
    if [[ ! -z $DEBUG ]]; then
        echo "-----"
        echo "WEB: http://www.pawnee.kansasgis.com/ParcelDetail.aspx?parcel=${PID}&quickref=R2"
    fi

    WEB=$(curl -s "http://www.pawnee.kansasgis.com/ParcelDetail.aspx?parcel=${PID}&quickref=R2" -b /tmp/make_pawnee_pass_cookies.txt)
