...
#PID => Address
#http://www.geary.kansasgis.com/ParcelDetail.aspx?parcel={PARCEL}&quickref=R2
...
curl -s 'http://www.geary.kansasgis.com' -c /tmp/make_geary_cookies.txt &>/dev/null
curl -s -X POST -d "" 'http://www.geary.kansasgis.com/Disclaimer.aspx' -b /tmp/make_geary_cookies.txt -c /tmp/make_geary_pass_cookies.txt &>/dev/null

while read -r LINE; do
    CENTRE=$(echo $LINE | jq -r -c '.geometry | .coordinates' | sed -e 's/\[//' -e 's/]//')
    PID=$(echo $LINE | jq -r -c '.properties | .PIN19')
    ...
    if [[ ! -z $DEBUG ]]; then
        echo "-----"
        echo "WEB: http://www.geary.kansasgis.com/ParcelDetail.aspx?parcel=${PID}&quickref=R2"
    fi

    WEB=$(curl -s "http://www.geary.kansasgis.com/ParcelDetail.aspx?parcel=${PID}&quickref=R2" -b /tmp/make_geary_pass_cookies.txt)
